...
#将部分数据打印出来
#将原表的数据和计算出的数据一起写入Clothes.xlsx文件
...
for s in range(nrows):
    for h in range(ncols):
        le = sheet.cell(s,h).value
        in_sheet.write(s,h,le)
...
for clothing_name in clothing_Sale:
    ...
    print(percentage)
    in_sheet.write(i,4,percentage) #第i行第4列
    i+=1
in_sheet.write(nrows+1,0,sales_volume)
in_sheet.write(nrows+1,3,avg_sale)
